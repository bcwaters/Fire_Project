...
const RegionAcresChart = ({ regionId, regionName, isMobile = false }) => {
	...
	useEffect(() => {
		...
		const processedData = data.map((incident) => ({
			...
			totalAcres:
				parseFloat((incident["Total Acres"] || "0").replace(",", "")) ||
				0,
			containedPercent: parseInt(incident["%"]) || 0,
			personnel:
				incident["Total PPL"] === "UNK"
					? 0
					: parseInt(
							(incident["Total PPL"] || "0")
								.split("/")[0]
								.replace(",", "")
					  ) || 0,
			changePersonnel:
				parseInt((incident["Chge in PPL"] || "0").replace(",", "")) ||
				0,
	...
	if (error) {
		return (
			<div className="error-message">
				Chart not available for {regionName}
			</div>
		);
	}
	...
	if (!data || data.length === 0) {
		return (
			<div className="error-message">
				No data available for {regionName}
			</div>
		);
	}
