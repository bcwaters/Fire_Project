		...
		async function fetchData() {
			...
			try {
				...
				const dataResp = await fetch(
					`/data/${today}/regions/Region_${regionId}_${today}.json`
				);
		...
		async function fetchRegionSummary() {
			...
			try {
				const resp = await fetch(
					`/data/${today}/regions/region_summaries_${today}.json`
				);
				...
				const summaryLinesWithNewLines = summaryLines.map((line) =>
					line.replace(/^\s*$/, "\n\n")
				);
			...
			<div className="p-3">
				<div>
					<div className="flex justify-between items-center pb-1 ">
						<h1 className="font-bold text-2xl">{regionName}</h1>
						<button
					...
					<div className=" border rounded p-1">
						<div className="font-bold  text-gray-800">Summary</div>
						<p className="italic">{regionSummary.join("")}</p>
					</div>
				</div>
				...
				<div className="predictive-summary-container">
					<h2 className="predictive-summary-label">
						Region Summary:
					</h2>
					{summaryLoading && (
						<p className="predictive-summary-loading">
							Loading region summary...
						</p>
					)}
					{summaryError && (
						<p className="predictive-summary-error">
							{summaryError}
						</p>
					)}
					{!summaryLoading && !summaryError && regionSummary && (
						<pre className="predictive-summary-text">
							{regionSummary.join("")}
						</pre>
					)}
					{!summaryLoading && !summaryError && !regionSummary && (
						<p className="predictive-summary-error">
							No summary available for this region.
						</p>
					)}
				</div>
			</div>
