		...
		fetch(`/data/${today}/daily_summary.json`)
			...
			.then((data) => {
				...
				let cleanSummary = data.summary
					.replace(/Understanding the IMSR\s*/g, "")
					.replace(/IMSR Map\s*/g, "");
				// Remove "Fire Activity and Teams Assigned Totals"
				cleanSummary = cleanSummary.replace(
					/Fire Activity and Teams Assigned Totals\s*/g,
					""
				);
				// Remove the Comp fires block
				const compFiresRegex =
					/Fires not managed under a full suppression strategy[\s\S]*?can be found in the NWCG glossary  or here/;
	...
	return (
		<main className="">
			...
			<div className="welcome-section">
				...
				<div className="block md:hidden">
					...
					{isSummaryExpanded && (
						<div className="predictive-summary-text expanded">
							<pre className="predictive-summary-text mobile-header-content">
								{summary}
							</pre>
						</div>
					)}
				...
				<pre className="hidden md:block">{summary}</pre>
			</div>
			{/* Daily Summary Block */}
