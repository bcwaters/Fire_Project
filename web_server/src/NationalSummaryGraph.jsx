...
const NationalSummaryGraph = ({ data, headerData }) => {
  ...
  useEffect(() => {
    ...
    // Chart 1: Cumulative Acres and Incidents by GACC (top-left)
    createAcresIncidentsChart(svg, processedData, subplotWidth, subplotHeight, 0, 0);
    
    // Chart 2: Personnel by GACC (top-right)
    createPersonnelChart(svg, processedData, subplotWidth, subplotHeight, subplotWidth + gap, 0);
    
    // Chart 3: Resources Comparison (bottom-left)
  ...
  const createAcresIncidentsChart = (svg, data, width, height, xOffset, yOffset) => {
    ...
    const y = d3.scaleLinear()
      .domain([0, d3.max(data, d => Math.max(d.totalAcres, d.incidents))])
      .range([height, 0]);

    // Add bars for cumulative acres
    ...
    chartGroup.selectAll('.incidents-bar')
      .data(data)
      .enter()
      .append('rect')
      .attr('class', 'incidents-bar')
...
const createPersonnelChart = (svg, data, width, height, xOffset, yOffset) => {
  ...
  const createDetailsTable = (svg, data, width, height, xOffset, yOffset) => {
    ...
    const headerData = ['GACC', 'Incidents', 'Acres', 'Crews', 'Engines', 'Helis', 'Personnel', 'Change'];
    ...
    const colWidth = width / headerData.length;
  ...
  if (!data || data.length === 0) {
    return <div>No data available for national summary.</div>;
  }

  return (
    <div className="national-summary-graph">
      <svg ref={svgRef}></svg>
    </div>
  );
};
